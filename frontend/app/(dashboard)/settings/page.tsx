...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'general' | 'members' | 'roles' | 'profiles' | 'custom-fields' | 'invitations' | 'audit'>('general');
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col text-slate-100">
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 flex flex-col lg:flex-row gap-8">
        ...
        <section className="flex-1 min-w-0">
          ...
          {activeTab === 'profiles' && <ProfilesTab canManage={canManage} />}

          {activeTab === 'custom-fields' && <CustomFieldsTab canManage={canManage} />}
        </section>
      </main>
    </div>
  );
